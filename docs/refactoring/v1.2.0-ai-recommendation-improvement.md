# v1.2.0: AI 추천 시스템 개선

## 버전 정보
- **버전**: 1.2.0
- **날짜**: 2025-02-06
- **목표**: 커밋 타입 및 Scope 추천 정확도 향상

## 문제점

### 1. 커밋 타입 분류 오류
**사용자 피드백**:
```
feat: add api-proxy-backed plugin 으로 내가 직접 커밋

📊 Recommended commit type: build
   Recommended scope: [app-config.local.yaml app-config.yaml packages plugins yarn.lock]
```

새로운 플러그인 추가가 `build`로 분류되는 문제

### 2. Scope 과도한 파일 나열
여러 디렉토리 변경 시 모든 파일 이름을 나열:
```
[app-config.local.yaml app-config.yaml packages plugins yarn.lock]
```
`api-proxy-backend`처럼 간결한 scope가 필요

## 원인 분석

### 커밋 타입 분류 오류 원인
- 단순 파일 개수 비교만으로는 커밋 타입 판단 불가
- 새로운 소스 파일이 다수 추가되었음에도 `build` 타입 분류
- 파일 유형(source/config/test)과 변경 유형(new/deleted/modified) 분석 부족

### Scope 과도한 파일 나열 원인
- 여러 디렉토리 변경 시 공통 경로 추출 로직 부재
- fallback 로직이 모든 파일 이름을 나열하도록 구현됨
- 최상위 디렉토리가 같을 때 구체적인 하위 경로 추출 실패

## 해결 방안

### 1. InferCommitType 함수 구현

**파일**: `internal/git/diff.go`

```go
func InferCommitType(files []git.FileChange) string {
    if len(files) == 0 {
        return "chore"
    }

    // 파일 유형 및 변경 유형 분석
    sourceFiles := 0
    configFiles := 0
    newSourceFiles := 0
    deletedFiles := 0
    hasDependencyChanges := false

    for _, file := range files {
        if file.IsDeleted {
            deletedFiles++
        }

        switch file.FileType {
        case git.FileTypeSource:
            sourceFiles++
            if file.IsNew {
                newSourceFiles++
            }
        case git.FileTypeConfig:
            configFiles++
            // 의존성 파일 확인
            if strings.Contains(file.Path, "package.json") ||
                strings.Contains(file.Path, "go.mod") ||
                strings.Contains(file.Path, "requirements.txt") ||
                strings.Contains(file.Path, "pom.xml") ||
                strings.Contains(file.Path, "Cargo.toml") ||
                strings.Contains(file.Path, "yarn.lock") ||
                strings.Contains(file.Path, "package-lock.json") {
                hasDependencyChanges = true
            }
        }
    }

    // 우선순위에 따른 타입 결정
    // 1. 새로운 기능 추가 (3개 이상의 새 소스 파일)
    if newSourceFiles >= 3 {
        return "feat"
    }

    // 2. 의존성 변경
    if hasDependencyChanges && (configFiles >= sourceFiles) {
        return "build"
    }

    // 3. 소스 파일 수정 (새 파일 없음)
    if sourceFiles > 0 && newSourceFiles == 0 {
        // 파일 변경 내용에서 버그 관련 키워드 확인
        for _, file := range files {
            if strings.Contains(strings.ToLower(file.Changes), "fix") ||
                strings.Contains(strings.ToLower(file.Changes), "bug") ||
                strings.Contains(strings.ToLower(file.Changes), "error") ||
                strings.Contains(strings.ToLower(file.Changes), "patch") {
                return "fix"
            }
        }
        // 버그 관련 키워드가 없으면 refactor로 간주
        return "refactor"
    }

    // 4. 파일 삭제
    if deletedFiles > 0 && sourceFiles == 0 {
        return "chore"
    }

    // 5. 설정 파일만 변경
    if configFiles > 0 && sourceFiles == 0 {
        return "build"
    }

    return "chore"
}
```

**특징**:
- 새로운 소스 파일이 3개 이상일 때 `feat` 타입
- 의존성 파일 변경 시 `build` 타입
- 버그 관련 키워드 감지 시 `fix` 타입
- 파일 유형별 가중치 분석

### 2. InferScopes 함수 구현

**파일**: `internal/git/diff.go`

```go
func InferScopes(files []git.FileChange) []string {
    if len(files) == 0 {
        return []string{}
    }

    // 모든 파일 경로 수집
    paths := make([]string, 0, len(files))
    for _, file := range files {
        paths = append(paths, file.Path)
    }

    // 가장 긴 공통 경로 찾기
    commonPrefix := findCommonPrefix(paths)

    if commonPrefix == "" {
        // 공통 경로가 없으면 첫 번째 파일의 최상위 디렉토리 사용
        parts := strings.Split(files[0].Path, string([]string{"/", "\\"})[0])
        if len(parts) > 1 {
            return []string{parts[0]}
        }
        return []string{}
    }

    // 공통 경로를 분리하여 scope 결정
    parts := strings.Split(commonPrefix, string([]string{"/", "\\"})[0])

    // 1단계: 전체 경로 사용 (예: plugins/auth)
    if len(parts) >= 2 {
        scope := strings.Join(parts, "/")
        return []string{scope}
    }

    // 2단계: 단일 디렉토리
    if len(parts) == 1 {
        return []string{parts[0]}
    }

    return []string{}
}

func findCommonPrefix(paths []string) string {
    if len(paths) == 0 {
        return ""
    }

    if len(paths) == 1 {
        return paths[0]
    }

    // 구분자 표준화
    normalized := make([]string, len(paths))
    for i, path := range paths {
        normalized[i] = strings.ReplaceAll(path, "\\", "/")
    }

    // 첫 번째 경로를 기준으로 비교
    first := normalized[0]
    minLen := len(first)

    for i := 1; i < len(normalized); i++ {
        if len(normalized[i]) < minLen {
            minLen = len(normalized[i])
        }
    }

    commonPrefix := ""
    for i := 0; i < minLen; i++ {
        char := first[i]
        allMatch := true
        for j := 1; j < len(normalized); j++ {
            if normalized[j][i] != char {
                allMatch = false
                break
            }
        }
        if !allMatch {
            break
        }
        commonPrefix += string(char)
    }

    // 마지막이 디렉토리 구분자이면 제거
    if strings.HasSuffix(commonPrefix, "/") {
        commonPrefix = commonPrefix[:len(commonPrefix)-1]
    }

    // 공통 경로를 마지막 구분자까지 잘라내기
    lastSeparator := strings.LastIndex(commonPrefix, "/")
    if lastSeparator != -1 {
        return commonPrefix[:lastSeparator]
    }

    return ""
}
```

**특징**:
- 가장 긴 공통 디렉토리 경로 사용
- 단일 scope 우선
- path 구분자 호환 (`/`, `\`)

### 3. 프롬프트 강화

**파일**: `internal/core/prompt.go`

#### 3.1 커밋 타입 설명 추가

```go
func getCommitTypeDescription(commitType string, lang string) string {
    descriptions := map[string]map[string]string{
        "feat": {
            "ko": "새로운 기능 추가",
            "en": "New feature addition",
        },
        "fix": {
            "ko": "버그 수정",
            "en": "Bug fix",
        },
        "build": {
            "ko": "빌드 시스템 또는 의존성 변경",
            "en": "Build system or dependency changes",
        },
        // ... 기타 타입
    }
    // ...
}
```

**출력 예시**:
```
추천 타입: feat (새로운 기능 추가)
```

#### 3.2 변경 패턴 분석 추가

```go
func analyzeChangePattern(files []git.FileChange, lang string) string {
    // 파일 유형 및 변경 유형 분석
    newFiles := 0
    deletedFiles := 0
    sourceFiles := 0
    configFiles := 0
    testFiles := 0

    for _, file := range files {
        if file.IsNew {
            newFiles++
        }
        if file.IsDeleted {
            deletedFiles++
        }
        // ... 파일 유형 분석
    }

    // 변경 패턴 결정
    if newFiles > 0 && deletedFiles == 0 && sourceFiles >= 3 {
        if lang == "ko" {
            return "새로운 기능/모듈 추가"
        } else {
            return "New feature/module addition"
        }
    } else if deletedFiles > 0 {
        // ... 기타 패턴
    }
    // ...
}
```

**출력 예시**:
```
변경 패턴: 새로운 기능/모듈 추가
```

#### 3.3 디렉토리 구조 분석 추가

```go
func analyzeDirectoryStructure(files []git.FileChange, lang string) string {
    dirStats := make(map[string]*dirInfo)

    for _, file := range files {
        parts := strings.Split(file.Path, "/")
        dir := parts[0]
        // ... 디렉토리별 파일 유형 집계
    }

    // 결과 생성
    for dir, info := range dirStats {
        if lang == "ko" {
            builder.WriteString(fmt.Sprintf(
                "- %s: 총 %d개 (소스: %d, 설정: %d, 테스트: %d, 문서: %d, 새 파일: %d)\n",
                dir, info.total, info.source, info.config, info.test, info.doc, info.newFiles))
        }
    }

    return builder.String()
}
```

**출력 예시**:
```
디렉토리 구조:
- plugins: 총 5개 (소스: 5, 설정: 0, 테스트: 0, 문서: 0, 새 파일: 5)
- packages: 총 3개 (소스: 2, 설정: 1, 테스트: 0, 문서: 0, 새 파일: 1)
```

## 결과

### 1. 커밋 타입 정확도 향상
**이전**:
```
feat: add api-proxy-backend plugin → build 타입 분류 (오류)
```

**개선 후**:
```
feat: add api-proxy-backend plugin → feat 타입 분류 (정확)
```

### 2. Scope 간결화
**이전**:
```
[app-config.local.yaml app-config.yaml packages plugins yarn.lock]
```

**개선 후**:
```
api-proxy-backend
```

### 3. LLM 판단 개선
- 추천 타입 + 설명 제공으로 LLM 이해도 향상
- 변경 패턴 분석으로 컨텍스트 강화
- 디렉토리 구조 분석으로 범위 명확화

## 관련 파일 변경

### `internal/git/diff.go`
- **추가**: `InferCommitType()` - 커밋 타입 추론
- **추가**: `InferScopes()` - Scope 추론
- **추가**: `findCommonPrefix()` - 공통 경로 찾기

### `internal/core/prompt.go`
- **수정**: `GeneratePrompt()` - 프롬프트 구조 강화
- **추가**: `getCommitTypeDescription()` - 타입 설명 반환
- **추가**: `analyzeChangePattern()` - 변경 패턴 분석
- **추가**: `analyzeDirectoryStructure()` - 디렉토리 구조 분석

## 다음 개선점

- [ ] 변경된 함수/클래스 명시 (diff에서 import/export 함수 추적)
- [ ] 성능 메트릭 추가 (토큰 수, 응답 시간)
- [ ] A/B 테스트 결과 수집 (사용자 피드백)
- [ ] 더 다양한 엣지 케이스 테스트
- [ ] 사용자 정의 타입/스코어 규칙 지원

## 회고

### 잘한 점
- ✅ 하이브리드 접근: 규칙 기반 추론 + AI 최종 판단
- ✅ 사용자 피드백 즉시 반영
- ✅ 점진적 개선 (단계별 구현)

### 개선할 점
- ⚠️ 엣지 케이스 테스트 더 필요
- ⚠️ 성능 최적화 (대규모 레포지토리)
- ⚠️ 사용자 정의 규칙 지원

### 학습한 점
- 📚 LLM에 더 풍부한 컨텍스트 제공이 효과적
- 📚 프롬프트 엔지니어링의 중요성
- 📚 사용자 피드백 주기가 빠를수록 좋음

## 참고

### 사용자 피드백 원문
```
1. 기능 추가로 feat: 가 더 중요한 커밋인데 build 로 분류됌, 
   같은 폴더에 없을때 괄호안에 모든 파일의 이름을 작성해버림..
   
   : feat: add api-proxy-backed plugin 으로 내가 직접 커밋
   
   📊 Recommended commit type: build
      Recommended scope: [app-config.local.yaml app-config.yaml packages plugins yarn.lock]
   
   정확성이 부족한느낌이야
```

### 관련 커밋
- `c5c3a54`: fix(diff): Refactor diff.go for improved performance
- `ad6ae4e`: Initial commit for AI recommendation improvement