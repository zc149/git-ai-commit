# v0.4.0: 메시지 반복 및 캐시 기능

## 개요

v0.4.0에서는 사용자 경험을 개선하기 위해 커밋 메시지 반복 생성 기능과 캐시 시스템을 도입했습니다. 사용자가 AI 추천 메시지에 만족하지 않을 때 새로운 후보를 생성하고, 만족하는 메시지를 찾으면 이를 캐시하여 다음에 같은 작업을 할 때 재사용할 수 있습니다.

## 동기

### 문제 1: AI 추천이 마음에 안 들 때
- 기존에는 첫 번째 추천만 제공되었음
- 마음에 안 들면 `c`를 눌러 직접 입력해야 함
- 여러 번 시도하려면 명령어를 다시 실행해야 함

### 문제 2: 반복 작업의 비효율성
- 같은 타입의 커밋을 반복할 때마다 같은 메시지를 직접 입력하거나 여러 번 후보를 확인해야 함
- 예: 버그 수정 후 테스트 코드 수정 패턴이 반복될 때

## 해결 방안

### 1. 재추천 기능 (Regenerate)
- 후보 메시지를 확인하고 마음에 안 들면 `r` 키를 눌러 새로운 후보 생성
- 횟수 제한 없이 계속 새로운 후보를 생성할 수 있음

### 2. 캐시 시스템
- 선택한 메시지를 diff hash 기반으로 저장
- 같은 diff로 다시 시도할 때 캐시된 메시지를 `p` 키로 선택 가능
- 캐시 위치: `~/.git-ai-commit/cache.json`

### 3. 컨트롤 플로우 개선
- Go의 에러 인터페이스를 활용한 제어 흐름
- `RegenerateError`: 재추천 요청을 나타내는 에러
- `UsePrevMessageError`: 이전 메시지 사용 요청을 나타내는 에러

## 설계 결정사항

### Diff Hash 기반 캐싱

**결정**: 캐시 키로 diff hash 사용

**이유**:
- **안전성**: diff 내용이 같으면 동일한 커밋 메시지를 사용해도 안전
- **정확성**: 파일 이름만으로는 변경 내용의 일치를 보장할 수 없음
- **유연성**: 같은 파일이라도 변경 내용이 다르면 다른 캐시로 저장

**구현**:
```go
func CalculateDiffHash(diff string) string {
    hash := sha256.New()
    hash.Write([]byte(diff))
    return hex.EncodeToString(hash.Sum(nil))
}
```

### 에러 기반 제어 흐름

**결정**: 커스텀 에러 타입을 사용하여 제어 흐름 처리

**이유**:
- Go의 에러 인터페이스를 자연스럽게 활용
- 에러 타입 체크를 통한 명확한 제어 흐름
- 코드의 가독성과 유지보수성 향상

**구현**:
```go
// 재추천 에러
type RegenerateError struct{}
func (e *RegenerateError) Error() string {
    return "regenerate requested"
}

// 이전 메시지 사용 에러
type UsePrevMessageError struct {
    Message string
}
func (e *UsePrevMessageError) Error() string {
    return "use previous message"
}

// 사용 예시
if _, ok := err.(*RegenerateError); ok {
    // 새로운 후보 생성
}
if prevMsgErr, ok := err.(*ui.UsePrevMessageError); ok {
    selectedMessage = prevMsgErr.Message
}
```

### 캐시 저장소

**결정**: 파일 시스템에 JSON 형식으로 저장

**이유**:
- **단순성**: 별도의 데이터베이스나 의존성 불필요
- **이식성**: 홈 디렉토리를 사용하여 사용자별 캐시 관리
- **투명성**: JSON으로 쉽게 확인하고 수정 가능

**구현**:
```go
type CacheManager struct {
    cachePath string
}

type CacheData struct {
    DiffHash string    `json:"diffHash"`
    Message  string    `json:"message"`
    Timestamp time.Time `json:"timestamp"`
}
```

## 구현 세부사항

### 1. 캐시 매니저 (`internal/cache/cache.go`)

**주요 기능**:
- `NewCacheManager()`: 캐시 디렉토리 및 파일 생성
- `Load(diffHash)`: diff hash로 캐시된 메시지 로드
- `Save(diffHash, message)`: 선택한 메시지를 캐시에 저장

**캐시 구조**:
```json
{
  "entries": [
    {
      "diffHash": "abc123...",
      "message": "feat: add feature",
      "timestamp": "2024-02-09T17:00:00Z"
    }
  ]
}
```

### 2. UI 확장 (`internal/ui/selector.go`)

**새로운 옵션**:
- `r) Regenerate candidates`: 새로운 후보 생성 요청
- `p) Use previous message`: 캐시된 이전 메시지 사용 (이전 메시지가 있을 때만)

**메시지 포맷팅**:
```go
// 이전 메시지가 있을 때
p) 이전 메시지 사용 (feat: add feature)
1) feat: add feature
2) feat: implement feature
c) Custom input
r) Regenerate candidates
q) Quit

Select (p/1-2 or c/r/q): 
```

### 3. 메인 루프 수정 (`cmd/root.go`)

**재추천 루프**:
```go
for {
    selectedMessage, err = selector.Select(messages, prevMessage)
    
    if err != nil {
        // 재추천 요청
        if _, ok := err.(*ui.RegenerateError); ok {
            messages, err = generator.Generate(diffResult, detail, lang)
            continue
        }
        
        // 이전 메시지 사용
        if prevMsgErr, ok := err.(*ui.UsePrevMessageError); ok {
            selectedMessage = prevMsgErr.Message
            break
        }
        
        // 그 외 에러 (종료 등)
        return err
    }
    
    // 정상 선택
    break
}
```

**캐시 통합**:
```go
// 캐시 매니저 초기화
cacheManager, err := cache.NewCacheManager()
if err != nil {
    return fmt.Errorf("캐시 매니저 생성 실패: %w", err)
}

// 이전 메시지 로드
diffHash := git.CalculateDiffHash(diffResult.RawDiff)
cachedData, err := cacheManager.Load(diffHash)
var prevMessage string
if err == nil && cachedData != nil {
    prevMessage = cachedData.Message
}

// 선택한 메시지 캐시에 저장
if err := cacheManager.Save(diffHash, selectedMessage); err != nil {
    fmt.Println("⚠️ 커밋 메시지 캐시 저장 실패")
}
```

### 4. Diff Hash 계산 (`internal/git/diff.go`)

**함수 추가**:
```go
func CalculateDiffHash(diff string) string {
    hash := sha256.New()
    hash.Write([]byte(diff))
    return hex.EncodeToString(hash.Sum(nil))
}
```

## 사용 시나리오

### 시나리오 1: 후보가 마음에 안 들 때

1. `git-ai-commit` 실행
2. AI가 3개의 후보 메시지 생성
3. 후보들이 마음에 안 들음 → `r` 입력
4. 새로운 후보 3개 생성
5. 여전히 마음에 안 들음 → `r` 입력
6. 새로운 후보 3개 생성
7. 마음에 드는 후보 발견 → `1` 입력
8. 커밋 완료

### 시나리오 2: 반복 작업 효율화

1. 버그 수정 작업 수행 (패턴: 파일 변경 + 테스트 수정)
2. `git-ai-commit` 실행
3. 후보 확인 후 만족스러운 메시지 선택 → `2` 입력
4. 커밋 완료, 메시지 캐시에 저장
5. 다른 버그 수정 작업 수행 (같은 패턴)
6. `git-ai-commit` 실행
7. `p) 이전 메시지 사용 (fix: resolve bug)` 옵션 표시
8. `p` 입력하여 이전 메시지 재사용
9. 커밋 완료

## 장점

1. **사용자 경험 개선**: 마음에 안 드는 후보를 계속 재생성 가능
2. **시간 절약**: 반복적인 작업에서 이전 메시지를 재사용
3. **유연성**: 횟수 제한 없이 원하는 만큼 후보를 생성
4. **안전성**: diff hash 기반 캐싱으로 잘못된 메시지 재사용 방지
5. **단순성**: 파일 시스템 기반 캐시로 복잡한 의존성 없음

## 향후 개선 가능성

1. **캐시 만료**: 오래된 캐시 자동 삭제 기능
2. **캐시 관리**: 캐시 확인 및 삭제 명령어
3. **메시지 히스토리**: 선택했던 모든 메시지 히스토리 확인
4. **즐겨찾기**: 자주 사용하는 메시지 패턴 저장
5. **캐시 공유**: 팀 단위로 캐시 공유 (Git 기반)

## 테스트

### 수동 테스트 케이스

1. **재추천 기능**
   - 후보 확인 후 `r` 입력
   - 새로운 후보 생성 확인
   - 여러 번 반복 확인

2. **캐시 기능**
   - 메시지 선택 후 커밋
   - 같은 diff로 다시 시도
   - `p) 이전 메시지 사용` 옵션 확인
   - `p` 입력 후 캐시된 메시지 사용 확인

3. **캐시 무효화**
   - diff 변경 후 `git-ai-commit` 실행
   - `p` 옵션이 표시되지 않는지 확인

4. **옵션 조합**
   - 재추천 후 이전 메시지 사용
   - 직접 입력 후 캐시 확인

## 버그 수정

v0.4.0 개발 과정에서 발견하고 수정한 버그들입니다.

### 버그 1: detail 기본값이 medium으로 나오는 문제

**문제**: `-detail` 옵션 없이 실행할 때 기본값이 `low`가 아닌 `medium`으로 표시됨

**원인**: `cmd/root.go`에서 기본값을 `"medium"`으로 설정

**수정**:
```go
// 수정 전
var detail string
cmd.Flags().StringVar(&detail, "detail", "medium", "detail level: low, medium, high")

// 수정 후
var detail string
cmd.Flags().StringVar(&detail, "detail", "low", "detail level: low, medium, high")
```

---

### 버그 2: high 디테일 레벨에서 짧은 메시지가 나오는 문제

**문제**: `detail` 레벨을 `high`로 설정해도 짧은 한 줄 메시지만 표시됨

**원인**: `internal/llm/utils.go`의 `parseCommitMessages()` 함수가 번호로 시작하는 첫 번째 줄만 파싱하고, 나머지 내용은 무시

**수정**:
```go
// 수정 전: 첫 번째 줄만 파싱
msg := removeNumberPrefix(line)
if msg != "" {
    messages = append(messages, msg)
}

// 수정 후: 다음 번호나 빈 줄이 나올 때까지 계속 수집
msg := removeNumberPrefix(line)
i++
for i < len(lines) {
    nextLine := trimWhitespace(lines[i])
    
    // 빈 줄이거나 다음 번호가 나오면 중지
    if nextLine == "" || isNumberedFormat(nextLine) {
        break
    }
    
    // 들여쓰기된 줄이나 연속된 내용을 추가
    if msg != "" {
        msg += "\n" + nextLine
    } else {
        msg = nextLine
    }
    
    i++
}

if msg != "" {
    messages = append(messages, msg)
}
```

---

### 버그 3: feat를 build로 잘못 분류하는 문제

**문제**: 새로운 기능 추가 (새 소스 파일 8개)인데 `build`로 분류됨

**예시**:
```
feat: add api-proxy-backed plugin  (사용자가 직접 작성)
→ 시스템 추천: build(app-config.local.yaml, app-config.yaml, packages, plugins, yarn.lock)
```

**원인**: 
- `yarn.lock`이 의존성 파일로 감지되어 `build`에 +5 점 부여
- 새 소스 파일이 3개 이상이어도 `feat` 점수가 충분히 높지 않음
- 의존성 파일이 있으면 무조건 `build`에 점수를 부여하는 로직 때문에 `feat`와 경쟁

**수정**:
```go
// 1. 새 소스 파일이 3개 이상일 때 feat 점수 증가
if newSourceFileCount >= 3 {
    typeScore["feat"] += 20  // 수정 전: 15
}

// 2. 의존성 파일이 있더라도 새 소스 파일이 많으면 build 점수 감소
// (새 기능 추가에 수반된 의존성 업데이트인 경우)
if hasDependencyFile && newSourceFileCount >= 3 {
    typeScore["build"] -= 10  // 새로운 로직
    typeScore["feat"] += 10  // 새로운 로직
}
```

**효과**:
- 새로운 기능 추가에 수반된 의존성 업데이트는 `build` 대신 `feat`로 분류
- 의존성 파일만 단독으로 변경된 경우에는 여전히 `build`로 분류

---

### 버그 4: scope에 모든 파일 이름이 나오는 문제

**문제**: 소스 파일이 여러 디렉토리에 흩어져 있을 때 scope에 모든 최상위 디렉토리 이름이 나옴

**예시**:
```
📊 Recommended commit type: build
   Recommended scope: [app-config.local.yaml app-config.yaml packages plugins yarn.lock]
```

**원인**: 
- `InferScopes()` 함수가 모든 디렉토리를 집계
- 설정 파일, 테스트 파일 등의 디렉토리도 포함되어 과도한 결과

**수정**: `InferScopes()` 함수 완전 재작성

```go
// 수정 전: 모든 파일의 디렉토리 집계
for _, file := range files {
    path := file.Path
    parts := strings.Split(filepath.Clean(path), string(filepath.Separator))
    
    for i := 1; i < len(parts); i++ {
        dir := strings.Join(parts[:i], string(filepath.Separator))
        dirCounts[dir]++
    }
    
    // 설정 파일도 디렉토리 분류에 포함
    if file.FileType == FileTypeSource {
        sourceDirs[dir] = true
    } else if file.FileType == FileTypeConfig {
        configDirs[dir] = true
    }
}

// 수정 후: 소스 파일만 집계
topLevelDirs := make(map[string]int)
for _, file := range files {
    if file.FileType != FileTypeSource {
        continue  // 소스 파일만 처리
    }
    
    path := file.Path
    parts := strings.Split(filepath.Clean(path), string(filepath.Separator))
    if len(parts) > 0 {
        topDir := parts[0]
        topLevelDirs[topDir]++
    }
}

// 3개 이상의 다른 최상위 디렉토리에 소스 파일이 있으면 "multiple" 사용
if commonPrefix != "" {
    scopes = append(scopes, simplifyScopeName(commonPrefix))
} else if len(topLevelDirs) >= 3 {
    scopes = append(scopes, "multiple")
} else {
    // 소스 파일이 가장 많은 최상위 디렉토리 하나만 선택
    maxCount := 0
    var primaryDir string
    for dir, count := range topLevelDirs {
        if count > maxCount {
            maxCount = count
            primaryDir = dir
        }
    }
    if primaryDir != "" {
        scopes = append(scopes, simplifyScopeName(primaryDir))
    }
}
```

**효과**:
- 설정 파일, 테스트 파일은 scope 추론에서 제외
- 소스 파일이 있는 디렉토리만 고려
- 3개 이상의 다른 디렉토리에 소스 파일이 있으면 "multiple" 사용
- 그 외에는 소스 파일이 가장 많은 디렉토리 하나만 scope로 제공

---

### 버그 5: 대규모 커밋에서 Deadlock 발생 (치명적)

**문제**: 17개 이상 파일이 포함된 커밋에서 deadlock 발생

**에러 메시지**:
```
fatal error: all goroutines are asleep - deadlock!

goroutine 1 [sync.WaitGroup.Wait]:
sync.(*WaitGroup).Wait(0x1400010a918)
    internal/worker/pool.go:120

goroutine 40 [chan send]:
git-ai-commit/internal/worker.(*WorkerPool).worker(0x1400010a900, 0x0?)
    internal/worker/pool.go:60

goroutine 41-43 [chan send]:
(동일한 상태)
```

**원인**: Producer-Consumer 패턴 위반

**상황 분석**:
1. 메인 goroutine이 작업을 `input` 채널에 전송
   - 버퍼 크기: `workers*2` (예: 4 workers → 버퍼 8)
   - 9번째 작업 전송 시 버퍼가 가득 차서 **블록됨**

2. Worker goroutines들이 `output` 채널에 결과 전송 시도
   - 버퍼 크기: `workers*2` (예: 4 workers → 버퍼 8)
   - 워커들이 계속 결과를 생성하다가 `output` 버퍼도 가득 참
   - 워커들이 **`output` 전송에서 블록됨**

3. **Deadlock 발생**:
   - 메인 goroutine: `input` 전송 대기 (버퍼 꽉 참)
   - Worker goroutines: `output` 전송 대기 (버퍼 꽉 참)
   - 워커들이 `input`에서 읽지 못하므로 메인 goroutine의 작업도 처리되지 않음

**수정 전 코드**:
```go
func ParseDiffParallel(diff string, workers int) ([]ParsedFile, error) {
    // ...
    pool := NewWorkerPool(workers)
    pool.Start()
    
    // 1. 작업 제출 (메인 goroutine에서 순차적으로)
    for i, fileDiff := range fileDiffs {
        pool.Submit(fileDiff, i)  // ← 여기서 블록될 수 있음
    }
    
    // 2. 결과 수집 goroutine 시작
    resultChan := make(chan []ParsedFile, 1)
    go func() {
        results := pool.Results()
        resultChan <- results
    }()
    
    // 3. 작업 완료 대기
    pool.Close()
    pool.Wait()
    
    // 4. 결과 수집
    results := <-resultChan
    
    return results, nil
}
```

**수정 후 코드**:
```go
func ParseDiffParallel(diff string, workers int) ([]ParsedFile, error) {
    // ...
    pool := NewWorkerPool(workers)
    pool.Start()
    
    // 1. 결과 수집 goroutine을 먼저 시작!
    resultChan := make(chan []ParsedFile, 1)
    go func() {
        results := pool.Results()  // 즉시 output 채널에서 읽기 시작
        resultChan <- results
    }()
    
    // 2. 작업 제출
    for i, fileDiff := range fileDiffs {
        pool.Submit(fileDiff, i)  // 워커들이 결과를 보내면 즉시 소비됨
    }
    
    // 3. 작업 완료 대기
    pool.Close()
    pool.Wait()
    
    // 4. 결과 수집
    results := <-resultChan
    
    return results, nil
}
```

**해결 원리**:

1. **Consumer 먼저 시작**:
   - 결과 수집 goroutine이 `pool.Results()`를 호출하여 `output` 채널에서 읽기 시작
   - Consumer가 항상 준비된 상태

2. **Producer가 자유롭게 작동**:
   - 워커들이 결과를 `output` 채널에 전송하면 Consumer가 즉시 소비
   - `output` 버퍼가 계속 비어있으므로 워커가 블록되지 않음

3. **input 채널도 자유로워짐**:
   - 워커들이 블록되지 않으므로 `input` 채널에서 작업을 계속 꺼냄
   - 메인 goroutine이 `input` 채널에 계속 전송 가능
   - `input` 버퍼가 가득 차지 않음

**왜 이 방법이 정석인가?**

1. **Producer-Consumer 패턴의 기본 원칙**:
   ```
   Producer (워커들) ──→ output 채널 ──→ Consumer (결과 수집 goroutine)
   ```
   - Consumer가 먼저 채널에서 읽기 시작해야 Producer가 블록되지 않음

2. **버퍼리스 채널의 성질**:
   ```go
   ch := make(chan int)  // 버퍼리스
   ch <- 42  // ← 누군가 읽을 때까지 블록됨
   <-ch      // ← 누군가 보낼 때까지 블록됨
   ```
   - 버퍼리스 채널은 동기적임
   - 보내는 쪽과 받는 쪽이 동시에 준비되어야 함

3. **실무에서의 적용 예시**:
   ```go
   // Go 표준 라이브러리에서도 같은 패턴
   out := make(chan Result)
   
   // Consumer 먼저 시작
   go func() {
       for r := range out {
           // 결과 처리
       }
   }()
   
   // Producer 시작
   for _, item := range items {
       out <- processItem(item)
   }
   close(out)
   ```

**성능 영향**:
- CPU 사용량: 동일 (같은 작업을 수행)
- 메모리: 더 적음 (버퍼가 작아도 됨)
- 복잡도: 동일 (같은 수의 goroutine)
- **차이점: Deadlock 발생 여부**

**효과**:
- 대규모 커밋(17개 이상 파일)에서 안정적으로 작동
- Producer-Consumer 패턴의 정석적 구현
- 채널 버퍼 크기에 덜 의존적
- 더 안정적인 병렬 처리

---

## 관련 파일

- `internal/cache/cache.go`: 캐시 매니저 구현
- `internal/ui/selector.go`: UI 옵션 및 에러 타입
- `cmd/root.go`: 캐시 통합 및 재추천 루프
- `internal/git/diff.go`: diff hash 계산, 커밋 타입 추론, scope 추론
- `internal/llm/utils.go`: 커밋 메시지 파싱

## 참고

- [Conventional Commits](https://www.conventionalcommits.org/)
- [Go Error Handling Best Practices](https://go.dev/doc/tutorial/errors)
